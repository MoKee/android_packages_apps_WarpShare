...
public class ReceiverService extends Service implements AirDropManager.ReceiverListener {
    ...
    @Override
    public void onAirDropRequest(String name, List<String> fileNames, AirDropManager.ReceiverAcceptCallback callback) {
    ...
    public void onAirDropTransfer(String fileName, InputStream input) {
        ...
        try {
            Okio.buffer(Okio.sink(file)).writeAll(Okio.source(input));
            input.close();
        } catch (IOException e) {
